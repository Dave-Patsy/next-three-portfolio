...
import Style from '../../styles/transitions/shoe/shoeConfig.module.css'
...
function Picker() {
  ...
  return (
    <div style={{ display: snap.current ? "block" : "none" }}>
      <HexColorPicker className={Style.picker} color={snap.items[snap.current]} onChange={(color) => (state.items[snap.current] = color)} />
      <h1 className={Style.h1}>{snap.current}</h1>
    </div>
  )
}

export default function ShoeConfig() {
  return (
    <>
